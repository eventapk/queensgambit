...
class _ParticipantsScreenState extends State<ParticipantsScreen> {
    ...
    _subscription = query.snapshots().listen(
        ...
        if (previousUserIds.isNotEmpty) {
          ...
          for (String newUserId in newUserIds) {
            Participant newUser = loaded.firstWhere((p) => p.phone == newUserId);
  ...
  Future<String> generateAndUploadQR(String userId, String data) async {
    try {
      // Use phone_number (userId) as QR code data to match Firestore query
      final painter = QrPainter(
        data: userId, // Changed to use userId (phone_number)
      ...
      print('Generated and uploaded QR code for userId: $userId, URL: $qrUrl');
      return qrUrl;
    } catch (e) {
      print('Failed to generate or upload QR code: $e');
    ...
    try {
      ...
      for (final phone in selectedPhones) {
        ...
        try {
          ...
          print('Processing user: $userName ($phone) - Status: $currentStatus, Email: $userEmail');
          ...
          if (currentStatus == 'registered') {
            try {
              ...
              print('Failed to send registration email to $userName: $emailError');
              failCount++;
            ...
            try {
              final qrUrl = await generateAndUploadQR(
                phone, // Use phone as userId
                phone, // Use phone as QR data
              );
              await sendQREmail(
                toEmail: userEmail,
                qrUrl: qrUrl,
      ...
      if (successCount > 0 && failCount == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ Successfully processed $successCount users and sent emails'),
  ...
  void _viewParticipantDetails(Participant participant) {
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  Widget _iconBtn(IconData icon, Color color, double screenWidth, VoidCallback onPressed) {
    return InkWell(
      onTap: onPressed,
      child: Container(
  ...
  Widget _tagButton(String text, Color bgColor, Color borderColor, double screenWidth, {Color? border}) {
    return Container(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return _buildParticipantRow(
                                    filteredParticipants[index], screenWidth, isSmallScreen);
                              },
                            ),
                          ),
                        ],
  ...
  Widget _buildParticipantRow(Participant p, double screenWidth, bool isSmallScreen) {
